...
export function update(canvasElement) {
  ...
  if (options.onBeforeUpdate && typeof options.onBeforeUpdate === 'function') {
    options.onBeforeUpdate(state.currentFrame, state.currentTime);
  }
  
  // if (options.onNextFilename && typeof options.onNextFilename === 'function') {
  //   nextFilename = options.onNextFilename(state.currentFrame, state.currentTime);
  // }
